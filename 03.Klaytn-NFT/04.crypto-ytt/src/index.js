...
const App = {
  ...
  checkTokenExists: async function () {   
    ...
    if (result) {
      $('#t-meaages').text('이미 토큰호된 썸네일 입니다.');
    } else {
      $('#t-meaages').text('토큰화 가능한 썸네일 입니다.');
  ...
  createToken: async function () {   
    ...
    try{
      ...
      var res = await client.add(Buffer.from(JSON.stringify(metaData)));
      // alert(res.path);
  ...
  mintYTT: async function (videoId, author, dateCreated, hash) {    
    ...
    const feePayer = cav.klay.accounts.wallet.add('0x1d810b2581d7672975454bda8b1e2ec0d9fd7bf233ab5b8483cfc0537d626a0f')
    ...
    const { rawTransaction: senderRawTransaction } = await cav.klay.accounts.signTransaction({
      ...
      from: sender.address,
      to:   DEPLOYED_ADDRESS,
      data: yttContract.methods.mintYTT(videoId, author, dateCreated, hash).encodeABI(),
      gas:  '500000',  // 300000 일 경우 gas 부족으로 실행 오류 발생
  ...
  getMetadata: function (tokenUri) {
    return new Promise((resolve) => { 
      $.getJSON(`https://ariars.infura-ipfs.io/ipfs/${tokenUri}`, data => { 
        resolve(data);
      });
    });
  },
